...
function BookModal({ book, anchor, onClose }) {
  ...
  // Initial position: near click (anchor), then clamp to viewport after we know modal size.
  useLayoutEffect(() => {
    ...
    // Prefer opening above-left of the click so the header is visible
    const desiredLeft = (anchor?.x ?? vw / 2) - rect.width * 0.4;
    const desiredTop  = (anchor?.y ?? vh / 2) - 40;

    const left = clamp(desiredLeft, 8, vw - rect.width - 8);
    const top  = clamp(desiredTop,  8, vh - rect.height - 8);
    setPos({ left, top });
  }, [anchor?.x, anchor?.y]);
  ...
  const addComment = () => {
    if (!text.trim()) return;
    const entry = { name: name.trim() || 'Anonymous', text: text.trim(), ts: Date.now() };
  ...
  const onPointerMove = (e) => {
    ...
    const left = clamp(dragRef.current.origLeft + dx, 8, vw - rect.width - 8);
    const top  = clamp(dragRef.current.origTop  + dy, 8, vh - rect.height - 8);
    setPos({ left, top });
  };
  const onPointerUp = () => {
  ...
  const modal = (
    <div className="book-modal-backdrop" role="dialog" aria-modal="true" onClick={onClose}>
      <div
        ...
        style={pos.left != null ? { left: pos.left, top: pos.top } : undefined}
      >
        <div className="modal-header" onPointerDown={onPointerDown} role="button" aria-label="Move dialog" tabIndex={0}>
        ...
        <div className="modal-body">
          ...
          <div className="modal-right">
            ...
            {book.author && <p className="meta-author" style={{ margin: 0 }}>{book.author}</p>}
            {book.review && <p style={{ marginTop: 8, fontWeight: 600 }}>{book.review}</p>}

            <label className="desc-label">Your one-line takeaway (saved locally)</label>
            <textarea
            ...
            <div className="comments">
              ...
              {comments.length === 0 && <p style={{ color: '#666', margin: '4px 0 10px' }}>Be the first to comment.</p>}

              {comments.map((c, i) => (
                <div key={i} className="comment-item">
                  <div className="comment-name">{c.name}</div>
                  <div className="comment-time">{new Date(c.ts).toLocaleString()}</div>
                  <div style={{ gridColumn: '1 / -1' }} className="comment-text">{c.text}</div>
                </div>
              ))}
              ...
              <div className="comment-form">
                ...
                <button className="primary-button" type="button" onClick={addComment}>Post comment</button>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <>
      <div className="book-carousel">
        <button className="carousel-btn prev" onClick={() => scrollByViewport(-1)} aria-label="Previous books" disabled={!canPrev}>‹</button>
          ...
          <div className="carousel-track" ref={trackRef} tabIndex={0} aria-live="polite">
            {books.map((b, i) => (
              <div
                ...
                key={b.id || i}
                onClick={(e) => setSelected({ index: i, x: e.clientX, y: e.clientY })}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') setSelected({ index: i, x: window.innerWidth/2, y: window.innerHeight/2 });
                }}
        ...
        <button className="carousel-btn next" onClick={() => scrollByViewport(1)} aria-label="Next books" disabled={!canNext}>›</button>
      </div>
